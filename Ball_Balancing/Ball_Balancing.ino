...
enum pins {
  ...
  DIR_PIN_A = 3,   // Direction-Signal Input A
...
//stepper motors
AccelStepper stepperA(1, STEP_PIN_A, DIR_PIN_A);  // (driver type, STEP_PIN, DIR_PIN)
AccelStepper stepperB(1, STEP_PIN_2, DIR_PIN_2);  // (driver type, STEP_PIN, DIR_PIN)
AccelStepper stepperC(1, STEP_PIN_3, DIR_PIN_3);  // (driver type, STEP_PIN, DIR_PIN)
...
void setup() {
  ...
  digitalWrite(ENABLE_PIN, LOW);        //sets the drivers off initially
  delay(1000);                    //small delay to allow the user to reset the platform
  digitalWrite(ENABLE_PIN, LOW);         //sets the drivers on
